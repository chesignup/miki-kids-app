...
export function LetterTracing({
  ...
  const drawCanvas = useCallback(() => {
    ...
    currentLetter.path.forEach((point, index) => {
      ...
      ctx.fillStyle = isHit ? '#22C55E' : '#E5E7EB';
      ctx.fill();
  ...
  useEffect(() => {
    const updateSize = () => {
      if (containerRef.current) {
        ...
        const size = Math.min(rect.width - 32, rect.height - 32, 350);
  ...
  const handleLetterComplete = useCallback(() => {
    ...
    const newProgress = addStars(STARS_PER_LETTER);
  ...
  }, [busyRef, lock, unlock, currentLetter, letterIndex, onStarsUpdate, safeSetTimeout]);
    ...
    return (
      <div style={styles.screen}>
        ...
        <div style={styles.completeContainer}>
          ...
          <p style={styles.completeText}>כתבת את כל האותיות!</p>
    ...
    <div style={styles.screen}>
      ...
      <div style={styles.content}>
        <div style={styles.progressContainer}>
          ...
          <div style={styles.progressBar}>
            <div
              style={{
                ...styles.progressFill,
                width: `${((letterIndex + 1) / TRACING_LETTERS.length) * 100}%`
              }}
            />
          </div>
        </div>
        ...
        <div style={styles.letterInfo}>
          <span style={styles.letterName}>{currentLetter?.name}</span>
          <span style={styles.letterChar}>{currentLetter?.letter}</span>
        </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  playAgainButton: {
    ...
    borderRadius: '24px',
    cursor: 'pointer',
    boxShadow: '0 4px 0 #16A34A, 0 6px 12px var(--shadow)'
  }
};
